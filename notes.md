...
2. Abstraction - hiding of data and shows neccessary data only to authorized user
3. Polymorphism - compile time,runtime  (method overloading,method overriding,methodhiding)
...
method hiding: method hiding in java occurs when a static method in a subclass has the same signature as static method in its super class in such cases the subclass methods does the superclass method rather than override it
this is because static method are associated with the class rather than instance so polymorphism does not not apply to static method

ABSTRACT CLASS: 
1. abstract keyword is used to create abstract class
2. you can't create object of abstract class
3. can have abstract as well as non abstract(concrete method)
4. doesn't support multiple inheritance
5. can have final,non final,static and non static variable
6. can provide implementation of interface
7. can have class members like private,public, protected etc..

Abstract method:
1. Can be declared only in an abstract class
2. Does not have a body
3. Can have public, protected, or default access
4. Cannot be private, because it must be overridden(initialized) in a child class

A package is a namespace that groups related classes and interfaces into a directory (folder)

Java Initialization block: 
    1. instance initilaization block: : runs first when object of the class is created

    {

    }

    2. static initialization block: does need object to be created static block runs when class is loaded (creates only one copy - runs 1st time only even if the object is created twice)
...
we cannot reassign value to final variable
types of final variable: 
1. final instance variable- at time of declaration,in constructor,in initialization block
2. final static variable - at the time of declaration, static initialization block,not in constructor
3. final local variable - at the time of declaration

int--->Integer: Boxing
Integer--->int: unboxing
Integer u=6; : autoboxing

Integer x=Integer.valueOf(123); //primitive to wrapper
...
INTERFACE:
    methods: all methods are by default public abstract
    variable: public, static ,final

interface gets inherited in class but class cannot be inherited in interface,
if interface inherit interface/class inherit classs we use extends otherwise we use implements

interface A
{
    public void fun1();
    //private void fun2(); not valid
    //protected void fun3(); not valid
...
//interface reference can point to objects of its implementing class (Ex - I i1=new Ex1();)
